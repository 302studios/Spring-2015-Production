...
public class comicControls : MonoBehaviour {
	...
	void Update () {
		...
		if (introDone && (Input.GetKeyDown (KeyCode.D) || Input.GetKeyDown (KeyCode.RightArrow))) {
		...
		} else if(introDone && (Input.GetKeyDown (KeyCode.A) || Input.GetKeyDown (KeyCode.LeftArrow))){
			if((currPanel > 0)) {
				currPanel--;
			}
		}
